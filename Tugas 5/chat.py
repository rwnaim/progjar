...
class Chat:
    ...
    def proses(self, data):
        ...
        try:
            ...
            if (command == 'auth'):
                ...
                sessionid = j[1].strip()
                # username = self.sessions[sessionid]['username']
                logging.warning("SHOW_USER: {}".format(sessionid))
                return self.get_show_user()
            elif (command == 'logout'):
                sessionid = j[1].strip()
                # username = self.sessions[sessionid]['username']
    ...
    def autentikasi_user(self, username, password):
        ...
        self.token.append(tokenid)
        self.sessions[tokenid] = {'username': username, 'userdetail': self.users[username]}
        return {'status': 'OK', 'tokenid': tokenid,'username':username}
    ...
    def get_show_user(self):
        ...
        for i in range(len(self.token)):
            msgs.append(self.sessions[self.token[i]]['username'])
        return {'status': 'OK', 'messages': msgs}
    ...
    def logout_user(self, token):
        ...
        logging.warning(self.token)
        logging.warning(self.sessions)
        msgs.append("logout berhasil")
        return {'status': 'OK', 'messages': msgs}
